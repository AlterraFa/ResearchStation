...
if __name__ == "__main__":
    ...
    dataset          = CarlaDatasetLoader("./data/recording_20250905_231635_best_spatial", downsize_ratio = 1, load_size = -1)
    train, val, test = dataset.split(train = 0.8, val = 0.2)
    train_loader     = DataLoader(train, batch_size = 50, shuffle = True, collate_fn = dataset.collate_fn)
    val_loader       = DataLoader(val, batch_size = 50, shuffle = True, collate_fn = dataset.collate_fn)
    
    mode  = "steer"
    run   = get_next_run("PilotNetExperiment")

    dummy = torch.zeros((1, 3, 209, 150)).to(gpu)
    model = PilotNetStatic(mode = mode, num_waypoints = 5, num_cmd = 4, droprate = 0.15).to(gpu) 
    model(dummy)  # Initialize dummy layers

    log_dir = f"PilotNetExperiment/run{run}"
    ...
    earlystop = EarlyStopping(20, 1e-5, path = f"./model/PilotNet/best_{model._get_name()}_run{run}.pt", verbose = True)
